...
if ( ! class_exists('PPCH_Modules_Settings')) {
    /**
     * class PPCH_Modules_Settings
     * Threaded commenting in the admin for discussion between writers and editors
     *
     * @author batmoo
     */
    class PPCH_Modules_Settings extends Module
    {
        ...
        public function __construct()
        {
            ...
            $args = [
                'title'                => __('Authors Settings', 'publishpress-checklists'),
        ...
        public function add_admin_scripts()
        {
            ...
            wp_enqueue_style('publishpress-modules-css', $this->module_url . 'lib/modules-settings.css', false,
                PUBLISHPRESS_CHECKLISTS_VERSION, 'all');
        ...
        {
            if ( ! isset($_POST['content_checklist_options'])) {
                return true;
            }

            $legacyPlugin = Factory::getLegacyPlugin();

            $enabledFeatures = $_POST['content_checklist_options']['features'];
        ...
        public function print_configure_view()
        {
            ...
            <form class="basic-settings"
                ...
                foreach ($legacyPlugin->class_names as $slug => $class_name) {
                    ...
                    echo '<input name="content_checklist_module_name[]" type="hidden" value="' . esc_attr($mod_data->name) . '" />';

                    $legacyPlugin->$slug->print_configure_view();
                }

                // Print the main module's settings page
                $legacyPlugin->content_checklist->print_configure_view();
                ...
                <?php if ($featuresCount > 0) : ?>
                    <div id="modules-wrapper">
                                    ...
                                    <?php foreach ($legacyPlugin->modules as $mod_name => $mod_data) : ?>
                                        ...
                                        <label for="feature-<?php echo esc_attr($mod_data->slug); ?>">
                                            <input id="feature-<?php echo esc_attr($mod_data->slug); ?>"
                                                   name="content_checklist_options[features][<?php echo esc_attr($mod_data->slug); ?>]" <?php echo ($mod_data->options->enabled == 'on') ? "checked=\"checked\"" : ""; ?>
                                                   type="checkbox">
                        ...
                        <?php echo '<input name="content_checklist_module_name[]" type="hidden" value="' . esc_attr($this->module->name) . '" />'; ?>
                    </div>
                <?php endif; ?>
